    ...
    data.forEach(function (series, i) {
      ...
      if (seriesItem && seriesItem.show && i == 0) {
        if (seriesItem.link) {
          ...
          seriesStr += '<dd data-value="' + String(series.parentid) + '" data-text="\u5168\u90E8\u8F66\u7CFB" data-target="series">\u5168\u90E8\u8F66\u7CFB</dd>';
        }
      }
      seriesStr += '<dt>' + String(series.fullname) + '</dt>';
      series.carlist.forEach(function (item) {
        seriesStr += '<dd data-value="' + String(item.id) + '" \n          data-text="' + String(item.fullname) + '" \n          ' + (item.initial ? 'data-pinyin=' + String(item.initial) : '') + '\n          data-target="series">' + String(item.fullname) + '</dd>';
      });
    });
    seriesStr += '</dl>';
...
Carpicker.prototype.setSpec = function (data, selectSpec) {
  ...
  if (data != undefined) {
      specStr += '<dt>' + String(data.name) + '</dt>';
        data.list.forEach(function (spec) {
