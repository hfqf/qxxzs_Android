...
public class InPurchaseAdapter extends BaseAdapter {
	private List<PurchaseRejectedInfo> list = null;
	private Context mContext;


	public InPurchaseAdapter(Context mContext, List<PurchaseRejectedInfo> list) {
	...
	public View getView(final int position, View view, ViewGroup arg2) {
		...
		if (view == null) {
			...
			view = LayoutInflater.from(mContext).inflate(R.layout.purchase_in_item, null);
...
//		viewHolder.code.setText("编码："+mContent.good_barcode);
	viewHolder.price.setText("库存："+mContent.num+"系统价格" +mContent.price);

//		viewHolder.num.setText("x"+mContent.num);
//
}else {
	viewHolder.name.setText("");
	viewHolder.name.setText("配件名：");
	viewHolder.price.setText("库存：" +"系统价格");
}
